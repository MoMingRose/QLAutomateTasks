...
class V2Free(BaseFileStorageTemplateForAccount):
    ...
    def fetch_primary_data(self, username: str, password: str, *args, **kwargs) -> bool | Tuple[str, any, bool]:
        ...
        try:
            ...
            if res_json["ret"] == 1 and res_json["msg"] == "登录成功":
                ...
                self.push_msg(f"{self._username}: 登录失败", res_json)
                return False
        except:
            self.push_msg(f"{self._username}: 登录失败", response.text)
            return False
    ...
    def sign_task_run(self, *args, **kwargs) -> bool:
        ...
        try:
            ...
            if res_json["ret"] == 1:
                ...
                self.push_msg(f"{self._username}: 签到成功, {msg}, 当前总流量: {traffic}")
                return True
            else:
                self.push_msg(f"{self._username}: 签到失败, {msg}")
                return False
        except:
            self.push_msg(f"{self._username}: 签到失败", response.text)
            return False
    ...
    def get_response(self, client_key: str, taskID: str):
        ...
        """
        self.push_msg("开始获取识别结果,每3秒请求一次，请耐心等待...")
        # 循环请求识别结果，3秒请求一次
        times = 0
        while times < 120:
            try:
                ...
                if res_json.get("errorId") == 0:
                    if res_json.get("status") == "ready":
                        self.push_msg("识别任务已完成，正在获取识别结果...")
                        if solution := res_json.get('solution', {}):
                            response = solution.get('gRecaptchaResponse')
                            if response:
                                self.push_msg("识别结果获取成功!")
                                return response
                            else:
                                self.push_msg("识别结果获取失败!")
                                return None
                        else:
                            self.push_msg("识别结果获取失败!")
                            return None
                    else:
                        self.push_msg("识别任务还在进行中，继续请求中...")
                else:
    ...
    def __fetch_data_sitekey(self, html=None):
        ...
        try:
            ...
            raise Exception("获取 data-sitekey 失败，今日签到任务或许已经完成!")
